...
| **ingest** | **Complete** — `SPEC.md`, 7× `docs/` (incl. MIGRATIONS) | `compose/`, worker `Dockerfile` | **Partial v0.36** — parsers + write + ACL + connectors + dedup | **33 contract/unit tests**; migrations 001–004 |
...
| `ingest/app/dedup_store.py` | Implemented v0.36 | Redis `dedup:{tenant}:{hash}` before embed |
