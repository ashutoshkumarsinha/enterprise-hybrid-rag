"""OpenAI-compatible embedding client for query-time dense vectors.

Spec: FR-13 single embed per request · IF-4 inference HTTP only.
"""
...
class EmbedClient:
    """Embed queries via hybrid-rag-inference or deterministic stub vectors."""
    ...
    def embed(self, text: str) -> list[float]:
        if self._stub:
            return _stub_dense_vector(text, self.dimension)
